...
public class CodeMD5 {
	...
	public CodeMD5() {
		...
		System.out.println(loopHashing("abc0", 2017));
	}
	
	private void loopHash() {
		Integer counter = 0;
		for (int i = 0; counter < 64; i++) {
			String hash = createMD5(joinString(i));
			String compareTo = checkThreeOfKind(hash);
			//System.out.println(compareTo + " jebne ");
			if (compareTo != null) {
				//System.out.println(compareTo + " i = " + i);
				...
				for (int j = i + 1; j <= i + 1000; j++) {
					...
					if (checkFiveOfKind(hashInner, compareTo) != null) {
						counter++;
						System.out.println(checkFiveOfKind(hashInner, compareTo) + " j = " + j + " i = " + i + " counter = " + counter);
					}
				}
